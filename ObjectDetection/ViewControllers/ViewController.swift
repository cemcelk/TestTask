...
extension ViewController: CameraFeedManagerDelegate {
  ...
  func didOutput(pixelBuffer: CVPixelBuffer, image: UIImage) {
    runModel(onPixelBuffer: pixelBuffer)
    changedImageView.image = image
  }
  ...
  func drawAfterPerformingCalculations(onInferences inferences: [Inference], withImageSize imageSize:CGSize) {
    ...
    for inference in inferences {
      ...
      let string = "\(inference.className)  (\(confidenceValue)%)"
        self.changedImageView.isHidden = true
      if confidenceValue > 50 && confidenceValue < 70 {
        let alertController = UIAlertController(title: "Detected", message: "the object is recognized with above 50% certainty. name: " + "\(inference.className)" , preferredStyle: .alert)
        let okAction = UIAlertAction(title: "OK", style: .cancel, handler: nil)
        alertController.addAction(okAction)
        ...
        overlayView.transform = CGAffineTransform(rotationAngle: CGFloat(Double.pi/2))
        changedImageView.isHidden = false
        let deadlineTime = DispatchTime.now() + .seconds(15)
        DispatchQueue.main.asyncAfter(deadline: deadlineTime) {
            self.cameraFeedManager.checkCameraConfigurationAndStartSession()
            self.previewView.transform = CGAffineTransform(rotationAngle: CGFloat(Double.pi*2))
